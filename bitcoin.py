...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0,len(Xi)):
        s = similarityIndex(X[:-1], Xi.iloc[i][:-1])
        exp = np.exp(wt*s)
        num += Xi.iloc[i][-1]*exp
        den += exp
    return num/den

def similarityIndex(a, b):
    mean_a = np.mean(a)
    mean_b = np.mean(b)
    std_a = np.std(a)
    std_b = np.std(b)
    ...
    for iterVar in range(0,len(a)):
        res+= (a.iloc[iterVar]-mean_a)*(b.iloc[iterVar]-mean_b)

    res/=(len(a)*std_a*std_b)
